...
- **`messageText[]` is Latin-1 encoded** (not UTF-8) — HTTP input is converted with `utf8ToLatin1()` first, then `expandIconTags()` is called (order matters: brackets in tags are ASCII and survive the encoding step).
